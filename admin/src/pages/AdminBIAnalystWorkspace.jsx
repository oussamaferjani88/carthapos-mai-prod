...
export default function AdminBIAnalystWorkspace() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        "Error loading: " + (err.response?.data?.error || err.message),
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight">
            BI Analyst Workspace
          </h1>
          <p className="text-sm text-muted-foreground">
            Review and analyze completed data uploads before dashboard creation.
          </p>
        </div>
                ...
                <tbody>
                  {requests.map((r) => (
                    <tr key={r.id} className="border-b hover:bg-muted/50">
                      ...
                      <td className="p-4">
                        {r.upload ? formatDate(r.upload.createdAt) : "\u2014"}
                      </td>
                      ...
                      <td className="p-4">
                        <div className="flex gap-1">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => navigate(`/bi-analysis/${r.id}`)}
                          >
