...
import {
  ...
  RefreshCw, ArrowLeft, FileText, UserCheck, Lightbulb, Database,
} from 'lucide-react';
...
export default function AdminBIAnalysisDetail() {
  ...
  const handleGenerateInsights = async () => {
    try {
      const res = await api.post(`/bi/analysis/${id}/generate-insights`);
  ...
  if (!analysis) {
    return (
      <div className="p-6">
        ...
        <Button variant="outline" className="mt-4" onClick={() => navigate('/bi-analysis')}>
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Button variant="ghost" size="sm" onClick={() => navigate('/bi-analysis')}>
              <ArrowLeft className="h-4 w-4 mr-1" /> Back
            </Button>
          </div>
          <h1 className="text-2xl font-bold">Analysis Detail</h1>
          <p className="text-muted-foreground text-sm">
            Client: {analysis.clientId} | Business: {analysis.businessType}
          </p>
        ...
        <div className="flex gap-2">
          ...
          {analysis.status === 'PENDING' && (
            <Button size="sm" onClick={() => handleStatusChange('UNDER_ANALYSIS')}>
          ...
          {(analysis.status === 'COMPLETED' || analysis.status === 'REJECTED') && (
            <Button variant="outline" size="sm" onClick={() => handleStatusChange('PENDING')}>
      ...
      {metrics && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Data from the analytics warehouse for this client.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              ...
              {metrics.totalSales !== undefined && (
                <MetricCard label="Sales Count" value={formatNumber(metrics.totalSales)} />
              )}
              {(metrics.averageTicket !== undefined) && (
                <MetricCard label="Avg Ticket" value={formatCurrency(metrics.averageTicket)} />
              )}
              {(metrics.avgTicket !== undefined) && (
                <MetricCard label="Avg Ticket" value={formatCurrency(metrics.avgTicket)} />
              )}
              {metrics.totalProducts !== undefined && (
                <MetricCard label="Products" value={formatNumber(metrics.totalProducts)} />
              )}
              {metrics.totalRows !== undefined && (
                <MetricCard label="Records" value={formatNumber(metrics.totalRows)} />
              )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <CardContent>
                    <div className="h-[220px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={metrics.revenueByDay} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
              ...
              {metrics.topProducts && metrics.topProducts.length > 0 && (
                <Card>
                  ...
                  <CardContent>
                    <div className="h-[220px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={metrics.topProducts.slice(0, 8)} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
                          <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="h-[200px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={metrics.peakHours} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
                          <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
              ...
              {/* Inventory */}
              {metrics.inventoryTurnover && metrics.inventoryTurnover.length > 0 && (
                <Card>
                  <CardHeader className="pb-2">
                  ...
                  <CardContent>
                    <div className="overflow-x-auto max-h-[200px] overflow-y-auto">
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {metrics.inventoryTurnover.slice(0, 15).map((item, i) => (
                            <tr key={i} className="border-b hover:bg-muted/50">
                              <td className="py-1 pr-2">{item.productName || item.name}</td>
                              <td className={`py-1 pr-2 ${(item.stock || 0) < 10 ? 'text-red-600 font-medium' : ''}`}>
                                {item.stock ?? '\u2014'}
                              </td>
                              <td className="py-1">{item.timesSold ?? '\u2014'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Record observations about this analysis.</CardDescription>
        </CardHeader>
        <CardContent>
          <Textarea
            value={notes}
