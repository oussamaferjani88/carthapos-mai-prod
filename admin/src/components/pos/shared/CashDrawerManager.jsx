...
export const CashDrawerManager = ({ onDrawerOpen, onCashCount, drawerStatus = 'closed' }) => {
  ...
  const calculateTotal = () => {
    const billsTotal = Object.entries(cashCount.bills).reduce((sum, [value, count]) => 
      sum + (parseFloat(value) * count), 0);
    const coinsTotal = Object.entries(cashCount.coins).reduce((sum, [value, count]) => 
      sum + (parseFloat(value) * count), 0);
    return billsTotal + coinsTotal;
  };

  const updateCount = (type, denomination, increment) => {
    setCashCount(prev => ({
      ...prev,
      [type]: {
        ...prev[type],
        [denomination]: Math.max(0, prev[type][denomination] + increment)
      }
    }));
  };
      ...
      <CardHeader className="pb-3">
        <CardTitle className="text-lg flex items-center gap-2">
          ...
          <span className={`ml-auto px-2 py-1 rounded text-sm ${
            drawerStatus === 'open' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
          }`}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className="flex items-center justify-center gap-2 p-3 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-all"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 p-3 bg-purple-500 hover:bg-purple-600 text-white rounded-lg transition-all"
          >
        ...
        {isCountMode && (
          <div className="border rounded-lg p-4 bg-gray-50">
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(cashCount.bills).map(([value, count]) => (
                  <div key={value} className="flex items-center gap-2 bg-white p-2 rounded">
                    ...
                    <button 
                      onClick={() => updateCount('bills', value, -1)}
              ...
              <div className="grid grid-cols-2 gap-1">
                {Object.entries(cashCount.coins).map(([value, count]) => (
                  <div key={value} className="flex items-center gap-1 bg-white p-1 rounded text-sm">
                    ...
                    <button 
                      onClick={() => updateCount('coins', value, -1)}
                    ...
                    <button 
                      onClick={() => updateCount('coins', value, 1)}
                      className="w-5 h-5 bg-green-500 text-white rounded text-xs hover:bg-green-600"
                    >+</button>
            ...
            <button
              onClick={() => {
                onCashCount?.(calculateTotal());
                setIsCountMode(false);
              }}
              className="w-full mt-3 p-2 bg-green-500 hover:bg-green-600 text-white rounded-lg flex items-center justify-center gap-2"
            >
        ...
        <div className="grid grid-cols-3 gap-2">
          <button className="flex flex-col items-center gap-1 p-2 bg-gray-100 hover:bg-gray-200 rounded text-xs">
