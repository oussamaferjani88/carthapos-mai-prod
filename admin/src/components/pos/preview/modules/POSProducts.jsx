...
const EMPTY_FORM = {
  name: '', family: '', price: '', cost_price: '', barcode: '', stock: '', min_stock: '', description: ''
};

export const POSProducts = ({ config, modules, setNotification }) => {
  const [products, setProducts] = useState(DEMO_PRODUCTS);
  const [families, setFamilies] = useState(DEMO_FAMILIES);
  ...
  const isBarcodeEnabled = modules ? moduleNames.some(n => n.includes('barcode')) : true;
  const isSupplierEnabled = modules ? moduleNames.some(n => n.includes('supplier')) : false;
  const isKitchenEnabled = modules ? moduleNames.some(n => n.includes('kitchen')) : false;
                  ...
                  <TableRow key={product.id} className={`transition-colors ${selectedIds.has(product.id) ? 'bg-muted/50' : 'hover:bg-muted/30'}`} onClick={() => openEdit(product)}>
                    ...
                    <TableCell className="text-right">
                      {margin === null
                        ...
                        : <span className="font-medium" style={{ color: parseFloat(margin) > 0 ? '#22c55e' : '#ef4444' }}>{margin}%</span>}
                    </TableCell>
