...
import {
  Users, Plus, Search, User, Phone, Mail, MapPin, Star, Edit2, Trash2, Gift,
  ShoppingBag, TrendingUp, Award, Heart, DollarSign, Receipt
} from 'lucide-react';

export const POSCustomers = ({ config, setNotification }) => {
  ...
  const demoPurchases = [
    ...
    { id: 103, total: 32.00, payment_method: 'cash', created_at: '2025-05-25T09:45:00', item_count: 2 },
  ];

  const demoStats = { visit_count: 3, total_spent: 155.70, average_ticket: 51.90, total_discounts: 4.50 };
  const demoFavorites = [
    { id: 1, name: 'Café expresso', price: 2.50, total_qty: 8, times_bought: 5 },
    { id: 2, name: 'Croissant', price: 1.80, total_qty: 6, times_bought: 4 },
    { id: 3, name: 'Salade César', price: 12.50, total_qty: 3, times_bought: 3 },
  ];
  ...
  const openDetail = (customer) => { setDetailCustomer(customer); setDetailTab('profile'); setDetailDialog(true); };
                ...
                {filteredCustomers.map(c => {
                  ...
                  return (
                    <tr key={c.id} className="cursor-pointer hover:bg-muted/30" onClick={() => openDetail(c)}>
                      ...
                      <td className="px-4 py-3"><Button variant="outline" size="sm"><Edit2 className="h-3 w-3" /></Button></td>
                    </tr>
                  );
                })}
              </tbody>
