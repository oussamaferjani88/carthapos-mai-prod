...
export const POSCaisse = ({ config, setNotification }) => {
  ...
  const demoActiveShift = activeShift || null;
  ...
  const handleOpen = () => {
    setActiveShift({
      id: 99, user_name: 'Démo Admin', opening_float: parseFloat(openFloat) || 0,
      opened_at: new Date().toISOString(), status: 'open', cash_sales: 0, card_sales: 0
    });
    setOpenDialog(false);
    notify('Caisse ouverte avec ' + formatCurrency(openFloat));
  };
  ...
  const handleClose = () => {
    ...
    setCounts({});
    setCloseNote('');
    notify('Caisse fermée - Total compté: ' + formatCurrency(total));
  };

  const openDetail = (shift) => { setShiftDetail(shift); setDetailDialog(true); };
      ...
      <Dialog open={openDialog} onOpenChange={setOpenDialog}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="grid gap-2"><Label>Fond de caisse initial</Label><Input type="number" step="0.01" value={openFloat} onChange={e => setOpenFloat(e.target.value)} /></div>
          </div>
          <DialogFooter>
