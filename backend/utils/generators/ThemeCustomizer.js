...
const { getPublicKey } = require('../../src/services/license/keys');
...
class ThemeCustomizer {
   ...
   * Theming/branding is fully runtime-driven (app-config.json -> useAppConfig
   * -> useThemeApplier / POSConfiguration.getStyleVars), so the only
   * generation-time work left is writing that config file.
   */
  ...
  async updateAppConfig() {
    ...
    // Written pre-build; vite's copyElectronFiles plugin (vite.config.js) copies
    // this into dist/app-config.json as part of the build, so it doesn't need
    // writing here too.
    ...
    const appConfig = {
      license: {
        ...
        clientId: this.license.clientId || this.license.client?.id || null,
      ...
      security: {
        requireUSBLicense: ['USB', 'HYBRID'].includes((this.license.bindingType || '').toUpperCase()),
        licenseFileName: 'license.key',
        licensePublicKey: getPublicKey(),
        licenseServerUrl: process.env.LICENSE_SERVER_URL || 'http://localhost:3001'
      },
      // Embed the signed license file for MACHINE-bound POS so the installed
      // app can validate and self-activate on first run without a USB carrier.
      // Stored under its own key to avoid clobbering the `license` metadata block.
      ...(this.license.__signedFileContent
        ? { licenseFile: JSON.parse(this.license.__signedFileContent) }
        : {}),
      printer: {
        enabled: true,
        autoprint: config.printReceiptAuto || false,
    ...
    logger.debug('App configuration updated');
