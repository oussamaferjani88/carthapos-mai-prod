...
interface ComponentConfig {
  ...
  navigationItem: { label: string; icon: React.ComponentType<any>; order: number } | null;
  permissions: string[];
}
...
export class POSComponentRegistry {
  static components = new Map<string, RegistryEntry>();
  ...
  static init() {
    ...
    this.register('user-management', POSUserManagement, {
      ...
      navigationItem: { label: 'Utilisateurs', icon: Shield, order: 22 },
    });

    this.register('settings', POSSettings, {
      navigationItem: { label: 'Paramètres', icon: Settings, order: 99 },
    });
  }
  ...
  static getComponent(componentId: string) {
    return this.components.get(componentId);
  }

  static getAvailableComponents(enabledModules: string[] = []) {
    const available: ({ id: string } & RegistryEntry)[] = [];
  ...
  static getNavigationItems(enabledModules: string[] = []) {
    return this.getAvailableComponents(enabledModules)
      .filter(comp => comp.config.navigationItem)
      .map(comp => ({
        id: comp.id,
        ...comp.config.navigationItem,
        icon: comp.config.navigationItem!.icon,
      }));
  }

  static isComponentAvailable(componentData: RegistryEntry, enabledModules: string[]) {
    ...
    const normalizedEnabledModules = enabledModules.map(module => {
      const moduleName = typeof module === 'string' ? module : (module as any).name || (module as any).label || (module as any).id || '';
  ...
  static getPageRenderer(pageId: string, enabledModules: string[] = []) {
