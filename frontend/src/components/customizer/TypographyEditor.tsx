...
const getFontWeightDisplay = (weight: string) => {
  const displayMap: Record<string, string> = { '300': 'light', '400': 'normal', '500': 'medium', '600': 'semibold', '700': 'bold' };
  return displayMap[weight] || weight;
};

const TypographyEditor = ({ formData, setFormData }: TypographyEditorProps) => {
  return (
      ...
      <CardContent className="space-y-2">
        ...
        <div className="space-y-1.5">
          ...
          <Select
            ...
            onValueChange={(value) => setFormData({ ...formData, configuration: { ...formData.configuration, fontFamily: value } })}
          >
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center space-x-2">
            <Slider
              ...
              onValueChange={(value) => setFormData({ ...formData, configuration: { ...formData.configuration, fontSize: `${value[0]}px` } })}
              max={20} min={10} step={1}
              className="flex-1"
            />
