...
// Ported from admin/src/components/customizer/LayoutEditor.jsx (flat groups
// instead of colored boxes per section; also drops a leftover debug block
// the previous client version had). Same 4 tabs, same config keys.
const LayoutEditor = ({ formData, setFormData }: LayoutEditorProps) => {
  const [activeTab, setActiveTab] = useState('general');

  const handleNavbarPositionChange = (position: string) => {
  ...
  }: { label: string; description: string; checked: boolean; onCheckedChange: (c: boolean) => void }) => (
  ...
  const renderGroup = ({ title, icon, children }: { title: string; icon: ReactNode; children: ReactNode }) => (
