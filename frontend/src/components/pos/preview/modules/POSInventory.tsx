...
function getStockStatus(stock: number, minStock: number): { text: string; variant: 'default' | 'destructive' | 'outline' } {
...
const DEMO_PRODUCTS: InvProduct[] = [
...
// Ported from admin/src/components/pos/preview/modules/POSInventory.jsx —
// the stock-adjust dialog now actually mutates state and logs a movement
// (previously both its Confirm/Cancel buttons were the same no-op), plus
// real filters/sort/pagination, bulk actions, and a per-product details
// dialog with movement history.
export const POSInventory = ({ config, modules, setNotification }: POSInventoryProps) => {
  const [products, setProducts] = useState<InvProduct[]>(DEMO_PRODUCTS);
  ...
  const [detailsProduct, setDetailsProduct] = useState<InvProduct | null>(null);
  ...
  const openAdjust = (product: InvProduct) => {
  ...
  const openDetails = (product: InvProduct) => {
