...
const STATUS_BADGES: Record<string, { bg: string; text: string; icon: any; label: string }> = {
...
// Ported from admin/src/components/pos/preview/modules/POSTransfers.jsx —
// a toggleable "Nouveau Transfert" form and status-dependent actions
// (Valider/Annuler/Marquer comme Reçu) on top of the transfer history,
// replacing the previous 2-item static list.
export const POSTransfers = ({ config }: { config: any }) => {
  const [showNewTransfer, setShowNewTransfer] = useState(false);

  const transfers = [
    { id: 'TRF-001', from: 'Magasin Centre-Ville', to: 'Magasin Gare', product: 'Café Expresso', qty: 50, status: 'completed', date: '2025-10-02', time: '14:30' },
    { id: 'TRF-002', from: 'Entrepôt Central', to: 'Zone Commerciale', product: 'Croissants', qty: 100, status: 'pending', date: '2025-10-03', time: '09:00' },
    { id: 'TRF-003', from: 'Magasin Gare', to: 'Centre-Ville', product: 'Sandwichs', qty: 30, status: 'in-transit', date: '2025-10-03', time: '11:15' },
  ];
  ...
  return (
    <div className="h-full flex flex-col space-y-4 py-6 bg-gray-50" style={{ fontFamily: config.fontFamily }}>
      ...
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setShowNewTransfer(!showNewTransfer)} style={{ backgroundColor: config.primaryColor }}>
          + Nouveau Transfert
        </Button>
      </div>
        ...
        <Card style={styles.card}>
          ...
          <CardContent>
            ...
            <div className="flex gap-2 mt-4">
              <Button style={{ backgroundColor: config.primaryColor }}>Créer le Transfert</Button>
              <Button variant="outline" onClick={() => setShowNewTransfer(false)}>Annuler</Button>
            </div>
          </CardContent>
        </Card>
      )}
            ...
            {transfers.map((transfer) => (
              <div key={transfer.id} className="border rounded-lg p-4 hover:bg-gray-50 transition">
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div><div className="text-gray-500 text-xs mb-1">De</div><div className="font-medium">{transfer.from}</div></div>
                  <div><div className="text-gray-500 text-xs mb-1">Vers</div><div className="font-medium">{transfer.to}</div></div>
                  <div><div className="text-gray-500 text-xs mb-1">Produit</div><div className="font-medium">{transfer.product}</div></div>
                </div>

                <div className="mt-3 pt-3 border-t flex items-center justify-between">
                  <div className="font-semibold" style={{ color: config.primaryColor }}>Quantité: {transfer.qty} unités</div>
                  <div className="flex gap-2">
                    {transfer.status === 'pending' && (
                      <>
                        <Button size="sm" style={{ backgroundColor: config.primaryColor }}>Valider</Button>
                        <Button size="sm" variant="outline">Annuler</Button>
                      </>
                    )}
                    {transfer.status === 'in-transit' && (
                      <Button size="sm" style={{ backgroundColor: config.primaryColor }}>Marquer comme Reçu</Button>
                    )}
                  </div>
                </div>
              </div>
