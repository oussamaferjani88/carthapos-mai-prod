...
// Ported from admin/src/components/pos/preview/modules/POSMultiStore.jsx —
// store selector, per-store stats, stock-comparison matrix, recent
// transfers, performance bars, and low-stock alerts, replacing the
// previous 3-card static list.
          ...
          <Card key={store.id} style={styles.card}>
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`px-2 py-1 rounded text-xs ${store.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
                  {store.status === 'active' ? 'Actif' : 'Entrepôt'}
                </span>
              </div>
              <h3 className="font-semibold text-sm mb-1">{store.name}</h3>
              <div className="space-y-1 text-xs text-gray-600">
                <div className="flex justify-between"><span>Stock:</span><span className="font-medium">{store.stock} articles</span></div>
                <div className="flex justify-between"><span>Ventes:</span><span className="font-medium">{store.sales}</span></div>
              </div>
            </CardContent>
          </Card>
        ))}
        ...
        <Card style={styles.card}>
          ...
          <CardContent>
            <div className="space-y-3">
              ...
              <Button className="w-full mt-2" style={{ backgroundColor: config.primaryColor }}>
                Nouveau Transfert
              </Button>
            </div>
          </CardContent>
              ...
              {stores.filter((s) => s.status === 'active').map((store, idx) => (
                <div key={idx}>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div className="h-2 rounded-full" style={{ width: `${(idx + 1) * 25}%`, backgroundColor: config.primaryColor }} />
                  </div>
                </div>
              ))}
            </div>
