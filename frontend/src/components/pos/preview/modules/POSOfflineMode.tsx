...
// Ported from admin/src/components/pos/preview/modules/POSOfflineMode.jsx —
// real interactive state: the online/offline toggle actually flips the
// status card and the pending-transactions view, replacing the previous
// fully static "dernière synchro" stub.
  ...
  return (
    <div className="h-full flex flex-col space-y-4 py-6 bg-gray-50" style={{ fontFamily: config.fontFamily }}>
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setIsOnline(!isOnline)} style={{ backgroundColor: isOnline ? '#10b981' : '#ef4444' }}>
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              {isOnline ? <Cloud className="h-12 w-12 text-green-600" /> : <CloudOff className="h-12 w-12 text-orange-600" />}
              <div>
                <h3 className="text-2xl font-bold">{isOnline ? 'Système En Ligne' : 'Mode Hors-Ligne Actif'}</h3>
                <p className="text-sm text-gray-600">
                  {isOnline ? 'Toutes les fonctionnalités sont disponibles' : 'Les ventes seront synchronisées à la reconnexion'}
                </p>
              </div>
            </div>
      ...
      <Card style={styles.card} className="flex-1">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" disabled={isOnline === false} style={{ backgroundColor: config.primaryColor }}>
