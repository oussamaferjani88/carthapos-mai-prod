...
// Mirrors the shell of the real POS (pos-template/components/Layout.jsx) —
// ported to match admin/src/components/pos/preview/POSPreview.jsx:
//   header, navbar + active page, footer with live clock/status.
const POSPreview = ({ configuration = {}, modules = [], navbarPosition = 'left', isDragMode = false, onComponentSelect }: POSPreviewProps) => {
  ...
  // Default config — prioritizes the passed-in configuration, falls back to
  // a demo user so the header's user block always has something to render.
  ...
  // Filters modules by the simulated role (admin = everything)
