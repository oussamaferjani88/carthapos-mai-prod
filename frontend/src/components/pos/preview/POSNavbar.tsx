...
// Ported from admin/src/components/pos/preview/POSNavbar.jsx (sidebar mode
// with overlay panel). Items come from the registry, in the same order as
// the real POS's navigation.
export const POSNavbar = ({
  ...
  const renderOverlayNavbar = () => (
    <>
      {/* Icon rail (normal document flow) */}
      <div
      ...
      {isOverlayOpen && (
        <div className="fixed inset-0 bg-black/30 z-40 transition-opacity duration-300" onClick={() => setIsOverlayOpen(false)} />
      )}
      ...
      {isOverlayOpen && (
        <div className="fixed left-0 top-0 h-full w-64 z-50 shadow-2xl transition-all duration-300" style={{ backgroundColor: config.backgroundColor }}>
          <div className="px-4 py-4 border-b flex items-center justify-between" style={{ borderColor: config.accentColor + '20' }}>
            ...
            <button onClick={() => setIsOverlayOpen(false)} className="p-1 rounded hover:bg-gray-100">
              <X className="w-4 h-4" />
            </button>
          </div>
            ...
            {navigationItems.map((item: any) => {
              ...
              return (
                <button
                  ...
                  style={isActive ? { backgroundColor: config.primaryColor, color: '#ffffff' } : { color: config.textColor }}
                >
                  <Icon className="w-5 h-5 flex-shrink-0" />
                  <span className="font-medium flex-1">{item.label}</span>
                  <ChevronRight className={cn('w-4 h-4 transition-transform', isActive ? 'text-white/80' : 'text-gray-400 group-hover:text-gray-600')} />
                </button>
              );
            })}
          </nav>
          ...
          <div className="border-t px-4 py-3" style={{ borderColor: config.accentColor + '20' }}>
            <div className="flex items-center space-x-2 text-xs" style={{ color: config.textMutedColor }}>
              ...
              <div>
                ...
                {config.currentUser && (
                  <div className="text-xs mt-1">Connecté: {config.currentUser.name || config.currentUser.username}</div>
                )}
              </div>
            </div>
          </div>
  ...
  const renderTopNavbar = () => (
    <div
      ...
      style={{ backgroundColor: config.backgroundColor, borderColor: config.accentColor + '20', height: navbarHeight }}
    >
  ...
  const navbarContent = config.navbarPosition === 'top' ? renderTopNavbar() : renderOverlayNavbar();
